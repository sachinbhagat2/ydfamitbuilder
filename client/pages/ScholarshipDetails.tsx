...
import { ArrowLeft, Calendar, CheckCircle, ChevronRight, DollarSign, FileText, MapPin, Star } from "lucide-react";
...
const ScholarshipDetails = () => {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        toast({ title: "Failed to load", description: String(e?.message || e) });
      } finally {
        setLoading(false);
      }
    };
  ...
  const details = useMemo(() => {
    ...
    return {
      ...
      deadline: sch.applicationDeadline ? new Date(sch.applicationDeadline).toISOString().slice(0,10) : "-",
      status: sch.status === "active" ? "Open" : sch.status === "closed" ? "Closed" : "Open",
      applicants: sch.currentApplications || 0,
      category: Array.isArray(sch.tags) && sch.tags.length ? sch.tags[0] : "General",
      eligibility: Array.isArray(sch.eligibilityCriteria) ? sch.eligibilityCriteria : [],
      documents: Array.isArray(sch.requiredDocuments) ? sch.requiredDocuments : [],
      rating: 4.7,
    };
  }, [sch]);
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast({ title: "Application submitted", description: "You can track it in Progress" });
        navigate("/progress");
      }
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white shadow-sm border-b border-ydf-light-gray">
        <div className="px-6 py-4">
          <div className="flex items-center space-x-3">
            <Link to="/scholarships" className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
              <ArrowLeft className="h-5 w-5 text-gray-600" />
            </Link>
            <h1 className="text-xl font-bold text-gray-900">Scholarship Details</h1>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        {!loading && !details && <div className="text-gray-600">Scholarship not found.</div>}
        {!loading && details && (
              ...
              <div className="p-6 border-b border-ydf-light-gray">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <h2 className="text-2xl font-bold text-gray-900 mb-2">{details.name}</h2>
                    <p className="text-gray-700">{details.description}</p>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-bold text-ydf-deep-blue">{details.amount}</div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {details.eligibility.length === 0 && (
                        <div className="text-sm text-gray-600">No specific eligibility listed.</div>
                      )}
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {details.documents.length === 0 && (
                        <div className="text-sm text-gray-600">No documents specified.</div>
                      )}
