...
export function authorize(...required: string[]) {
  return async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    const userTypeOk =
      required.length === 0 || required.includes(req.user.userType);
    let roleOk = false;
    ...
    if (Array.isArray(userRoles)) {
      ...
      try {
        const roles = await (mockDatabase as any).listUserRoles(
          req.user.id as any,
        );
        roleOk = required.some((r) =>
          (roles || []).some((x: any) => x.name === r),
        );
      } catch {
        roleOk = false;
      }
